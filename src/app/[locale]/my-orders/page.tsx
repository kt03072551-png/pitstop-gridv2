...
export default function MyOrdersPage() {
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      case "VERIFYING_SLIP":
        return <span className="flex items-center gap-1.5 px-3 py-1 rounded-full bg-amber-500/20 text-amber-600 dark:text-amber-400 text-xs font-bold border border-amber-500/30"><Clock className="w-3.5 h-3.5" /> {t.myOrders.statusVerifying}</span>;
      case "APPROVED":
        return <span className="flex items-center gap-1.5 px-3 py-1 rounded-full bg-emerald-500/20 text-emerald-600 dark:text-emerald-400 text-xs font-bold border border-emerald-500/30"><CheckCircle2 className="w-3.5 h-3.5" /> {t.myOrders.statusConfirmed}</span>;
      case "PREPARING_PARTS":
        return <span className="flex items-center gap-1.5 px-3 py-1 rounded-full bg-blue-500/20 text-blue-600 dark:text-blue-400 text-xs font-bold border border-blue-500/30"><Warehouse className="w-3.5 h-3.5" /> {t.myOrders.statusPreparing}</span>;
      case "SHIPPED":
        return <span className="flex items-center gap-1.5 px-3 py-1 rounded-full bg-purple-500/20 text-purple-600 dark:text-purple-400 text-xs font-bold border border-purple-500/30"><Truck className="w-3.5 h-3.5" /> {t.myOrders.statusShipped}</span>;
      default:
        return <span className="flex items-center gap-1.5 px-3 py-1 rounded-full bg-slate-500/20 text-slate-600 dark:text-slate-400 text-xs font-bold border border-slate-500/30">{t.myOrders.statusUnknown}</span>;
    }
  };
      ...
      <div className="max-w-4xl mx-auto space-y-8">
        ...
        <div>
          ...
          <p className="text-sm font-semibold text-[#112D4E]/70 dark:text-[#85B5D9] mt-2">
            {t.myOrders.pageDesc.replace("{name}", user?.name.split(" ")[0] || "Customer")}
          </p>
        </div>

        {/* Orders List */}
        <div className="space-y-4">
          {isLoading && <div className="text-center font-mono text-[#3F72AF] dark:text-[#3282B8]">{t.myOrders.loadingOrders}</div>}
            ...
            <div key={order.orderId} className="bg-white dark:bg-[#0F4C75]/20 border border-[#DBE2EF] dark:border-[#0F4C75] rounded-2xl p-5 shadow-sm hover:shadow-md transition-shadow">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-4 pb-4 border-b border-[#DBE2EF] dark:border-[#0F4C75]/50">
                <div>
                  <div className="flex flex-wrap items-center gap-2 sm:gap-3 mb-1">
                    <span className="font-mono text-sm sm:text-lg font-bold text-[#112D4E] dark:text-[#BBE1FA] break-all">{order.orderId}</span>
                ...
                <div className="text-left sm:text-right">
                  <div className="text-sm font-semibold text-[#112D4E]/60 dark:text-[#85B5D9]">{t.myOrders.totalAmount}</div>
              ...
              <div className="flex flex-col sm:flex-row gap-6">
                <div className="flex-1 space-y-3">
                  <div>
                    <div className="text-xs font-semibold text-[#112D4E]/60 dark:text-[#85B5D9] mb-1">{t.myOrders.itemsSummary}</div>
                  ...
                  <div>
                    <div className="text-xs font-semibold text-[#112D4E]/60 dark:text-[#85B5D9] mb-1">{t.myOrders.fulfillmentMethod}</div>
